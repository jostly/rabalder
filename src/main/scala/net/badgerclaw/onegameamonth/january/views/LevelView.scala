...
class LevelView(factory: ResourceFactory, level: Level) extends View {
  ...
  val tiles = Map[Tile, Array[TextureRegion]](
    ...
    Explosion(1, Diamond) -> tilesBag.extract(2, 0),
    Explosion(2, Diamond) -> tilesBag.extract(3, 0),
    Explosion(3, Diamond) -> tilesBag.extract(4, 0),
    Explosion(4, Diamond) -> tilesBag.extract(5, 0),
    Explosion(5, Diamond) -> tilesBag.extract(6, 0),
